...
- **Reprocessing controls**: `/api/reparse` clears markdown rows and vector entries before re-queuing document processing. `mode:"all"` nukes both stores and schedules every non-blacklisted PDF; `mode:"selected"` accepts exact filenames along with case-insensitive substring/fuzzy matches against `filename` and `file_path`. The frontend settings page surfaces buttons for these flows, so keep the response shape stable (`queued`, `skipped`, `markdown_deleted`, `vector_reset`).
...
- **Settings UX**: `frontend/src/pages/Settings.jsx` now exposes reparse actions (all or newline/comma-delimited filenames) alongside blacklist management; keep toast summaries informative (`Queued N`, `Skipped -> reason`) when adjusting response payloads.
