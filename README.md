...
After running local checks or builds, wipe transient outputs so they never leak into commits and to keep disk usage predictable:

```bash
# Remove Python bytecode generated by compile runs
find backend -path "*/__pycache__" -type d -prune -exec rm -rf {} +

# Drop any frontend bundles copied during manual builds
rm -rf frontend/dist backend/app/static/static/assets

# Trim dangling Docker images and builder cache created during local testing
docker image prune -f
docker builder prune -f
```

Run these commands whenever you finish a local test/build cycle; they keep the workspace clean for the next iteration and avoid accidentally pushing compiled artefacts.
